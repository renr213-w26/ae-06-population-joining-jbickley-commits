...
population_contitent <- population |> left_join(continents, by = join_by(country == entity))
...
population_contitent |> filter(is.na(continent))
```
...
```{r}
population_contitent <- population |> mutate(country = case_when(
...
Asia then Africa are highest, lowest is probably Oceania  
...
population_contitent |> group_by(continent) |> summarise(total_pop = sum(population)) |> arrange(desc(total_pop))
```
...
population_contitent |> group_by(continent) |> summarise(total_pop = sum(population)) |> ggplot(aes(continent, total_pop)) + 
  geom_col()+
  theme_classic()
...
population_contitent |> group_by(continent) |> summarise(total_pop = sum(population)) |> ggplot() + 
  geom_point(aes(total_pop, continent))+
  geom_segment(aes(continent, yend = continent, x=0, xend= total_pop))+
